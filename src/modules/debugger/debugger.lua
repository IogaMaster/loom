...
--
--
-- -- This is a modfied version, it embeds into loom.debugger()
-- -- See the end of file for difference from main
-- -- Thanks @slembcke for providing this. You rock
...
local function format_stack_frame_info(info)
	...
	local name = (
		info.name and "'" .. dbg.COLOR_BLUE .. info.name .. dbg.COLOR_RESET .. "'"
...
local function compile_chunk(block, env)
	...
	if not chunk then
		dbg_writeln(dbg.COLOR_RED .. "Error: Could not compile block:\n" .. dbg.COLOR_RESET .. block)
	end
	return chunk
end
...
local function where(info, context_lines)
	...
	if source and source[info.currentline] then
		for i = info.currentline - context_lines, info.currentline + context_lines do
			...
			local line = source[i]
			if line then
				dbg_writeln(dbg.COLOR_GRAY .. "% 4d" .. tab_or_caret .. "%s", i, line)
			end
		end
	else
...
local function cmd_trace()
	...
	while true do
		...
		dbg_writeln(
			dbg.COLOR_GRAY .. "% 4d" .. dbg.COLOR_RESET .. tab_or_caret .. "%s",
			i,
...
-- The only difference between the regular and the embedded version
--return dbg
loom.debugger = dbg
